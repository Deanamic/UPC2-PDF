 ...
 * Description: lcp (x,y) = min(lcp(x,x+1),lcp(x+1,x+2)...lcp(y-1,y)) to answer queries with RMQ O(1)
 * Time: Build: $O(N \log N)$ where $N$ is the length of the string
 * for creation of the SA. LCP $O(\log N)$ It is not necessary to use raddixsort if the O(nlog^2 n) fits the time limit, one can just use stl sort.
 * Memory: Build $O(N \log N)$ (Can be optimised as in B you only use the previous row to compute one row. But then you cannot do lcp
 * Status: Tested on https://www.codechef.com/problems/TASTR
 */
#pragma once
